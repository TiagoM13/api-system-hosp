...
const userRoutes = async (app: FastifyInstance) => {
    ...
    app.get("/users/:userId", { preHandler: checkPermissions([0]) }, (req, res) => getUserFactory().handle(req, res))
    app.post("/users", { preHandler: checkPermissions([0]) }, (req, res) => createUserFactory().handle(req, res))
    app.put("/users/:userId", { preHandler: checkPermissions([0]) }, (req, res) => updateUserFactory().handle(req, res))
